    ...
    if (location.length != 0) {
        ...
        for (let i = 0; i < content.length; i++) {
            ...
            if (fs.lstatSync(path.join(location, content[i])).isFile()) {
                ...
                li.classList.add('w-100', 'd-flex', 'align-items-center', 'text-white', 'cursor-pointer', 'file-el');
                const el = `<img src="./file.png" alt="file" id="icon" style="width: 30px"><span id="name">${content[i]}</span>`;
                li.innerHTML = el;
                ...
                const li = document.createElement('li');
                li.classList.add('w-100', 'd-flex', 'align-items-center', 'text-white', 'cursor-pointer', 'file-el');
                const el = `<img src="./folder.png" alt="folder" id="icon" style="width: 30px"><span id="name">${content[i]}</span>`;
                li.innerHTML = el;
                parent.appendChild(li);
                li.addEventListener('click', () => {
                    // Open folder again
